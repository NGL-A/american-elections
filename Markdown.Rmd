...
```{r, warning=FALSE, message= FALSE}
#logistic regression
logit.out <- glm(Party20~., data = data[-c(1:13,19:20,27,29,31,33,52,53)], family = binomial)
summary(logit.out)
#plot(logit.out)
library(pROC)
logistic.prob <- predict(logit.out, type = "response")
roc.out <- roc(Party20, logistic.prob, levels= c("No", "Yes"))
...
#plot(balance, income, col=l+1, pch=l*500+1)
...
# Set threshold = 0.5, confusion matrix
