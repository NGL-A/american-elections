...
From these two maps is easy to see that our initial intuition was right: the counties with a higher presence of the Black ethnicity have a democratic leaning, but there are many other counties that have a more modest ethnic representation which voted Republican. The state result are based on all citizens votes so it can be a bit misleading using the state result to make assumption on their counties because the counties contribution is based on their population.  
...
In the state of New York the Democratic party won with a big lead of votes, but watching the results map and the table they actually won in 13 counties against the 49 of the republicans, but from the second map is easy to see that most of the population is concentrated in the counties that form the city of New York that have a democratic leaning while the republican counties (except the south-eastern ones) count a really low population with respect to the most urbanized areas.
