...
In this section we are going to see how ours features are distributed along the population.
We start with three pie chart that represent three of the categories considered in the census of 2017: ethnicity, employment sector and usual method of transport.
...
As expected the dominant ethnicity is the White one that represent 63% of the total population of the United States, the other mayor ethnicities are the Hispanic ethnicity 18.32%, Black ethnicity 12.44% and Asian ethnicity 5.44%.
...
We can see that every class has a good representation, later we will see if a certain class of workers prefers a specific party.
...
The transport data correlates to the states geographical morphology: outside the big cities distances became very big so a use of a car by over the 90% of the population (Drive and Carpool) is justified.
...
This histogram represent the size of the counties, we can see that most of the counties count less than 100 000 inhabitants and the counties with over 1 000 000 inhabitants are very few. 
...
We know that he election were won by the Republican party in 2020, but, as we just saw, they won in almost a fifth of counties, this fact suggest that the distribution of the population is not the same between republican counties and democrat counties. Now we are going to investigate this fact. 
...
plot_usmap(data = FIPS.data, values = "Unemployment", include = c("AL", "AR", "GA", "LA", "MS", "NC", "SC", "TN")) + 
  scale_fill_continuous(low = "white", high = "red", name = "Unemployment (2017)", label = scales::comma) + 
  theme(legend.position = "right")
