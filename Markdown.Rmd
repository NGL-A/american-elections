...
```{r, results='hide'}
df <- data[-c(1:13,19:20,27,29,31,33,46,52:53)] #for the regressions
summary(df)
```
&ensp;&ensp;  
...
mod.out <- glm(Party20 ~. , data=data[-c(1:13,19:20,27,29,31,33,46,52,53)], family = binomial)
...
The Roc curve is a graphical representation of what happens in the previous table. The AUC (Area Under Curve) always has good values (close to 1) and is correctly shaped, meaning our predictions are fairly accurate. We also find the best threshold value to be at 0.8739469.
...
Let us use the cross-validation method to choose the optimal value of lambda.
...
We took New York as an example to show how the urban parts of the state are more inclusive and tend to have more progressive ideas voting the democrat party. The peripheral areas instead, and to have more traditional views and prefer the republican party. 
